...
var aws = require('aws-sdk'),
    ...
    impl = {
        ...
        buildSNSParams: function(topicArn, subject, report) {
            var snsParams = {
							TopicArn: topicArn,
							Subject: subject,
							Message: report
        ...
        SNSPlugin: function(scriptConfig, eventEmitter) {
           var self = this,
                reportError = function (err) {
                    if (err) {
                        console.log('Error publishing to SNS topic:', err);
                    }
                };
            self.config = JSON.parse(JSON.stringify(scriptConfig.plugins[constants.PLUGIN_NAME]));
            eventEmitter.on('done', function (report) {
								snsParams = impl.buildSNSParams(self.config[constants.PLUGIN_PARAM_TOPICARN], self.config[constants.PLUGIN_PARAM_SUBJECT] || "Artillery Report", report);
								sns.publiush(snsParams, reportError);
                console.log('Report published to SNS topic %s', self.config[constants.PLUGIN_PARAM_TOPICARN]);
            });
        }
    },
    api = {
